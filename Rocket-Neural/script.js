...
const COLLECTION_RADIUS = 40; // Radius of the collection area
...
const COLLECTION_MAX_SPEED = 0; // Velocidade máxima da coleta
const COLLECTION_ACCELERATION = 0; // Taxa de aceleração da coleta
const SUDDEN_MOVEMENT_CHANCE = 0; // Chance de movimento brusco a cada frame
...
function updateCollectionPosition(deltaTime) {
    ...
    const CENTER_MARGIN = 0.1; // 10% de margem nas bordas

    // Aumentar a chance de movimento brusco
    if (Math.random() < SUDDEN_MOVEMENT_CHANCE * 2) {
        collection.targetSpeedX = (Math.random() - 0.5) * COLLECTION_MAX_SPEED * 3;
        collection.targetSpeedY = (Math.random() - 0.5) * COLLECTION_MAX_SPEED * 3;
    }

    // Aumentar a aceleração
    collection.speedX += (collection.targetSpeedX - collection.speedX) * COLLECTION_ACCELERATION * 1.5;
    collection.speedY += (collection.targetSpeedY - collection.speedY) * COLLECTION_ACCELERATION * 1.5;

    // Aumentar a velocidade máxima
    const maxSpeed = COLLECTION_MAX_SPEED * 1.5;
    collection.speedX = Math.max(-maxSpeed, Math.min(maxSpeed, collection.speedX));
    collection.speedY = Math.max(-maxSpeed, Math.min(maxSpeed, collection.speedY));
...
function initializeGame() {
    ...
    // Mover a inicialização de centralCollection para antes de chamar resizeCanvas
    window.centralCollection = {
    ...
    // Agora que centralCollection está definida, podemos chamar resizeCanvas novamente
    resizeCanvas();
...
function resizeCanvas() {
    ...
    // Só recalcular os elementos do jogo se centralCollection já estiver definida
    if (window.centralCollection) {
        recalculateGameElements();
    }
}

function recalculateGameElements() {
    // Verificar se centralCollection existe antes de acessá-la
    if (window.centralCollection) {
        // Ajustar tamanho e posição da coleta central
        const centralCollection = window.centralCollection;
        centralCollection.width = CANVAS_WIDTH * 0.1;
        centralCollection.height = CANVAS_HEIGHT * 0.1;
        centralCollection.x = (CANVAS_WIDTH - centralCollection.width) / 2;
        centralCollection.y = (CANVAS_HEIGHT - centralCollection.height) / 2;
    }

    // Ajustar tamanho dos foguetes
    const rocketWidth = CANVAS_WIDTH * 0.05;
    const rocketHeight = CANVAS_HEIGHT * 0.05;
    if (rockets) {
    ...
    // Ajustar outros elementos do jogo conforme necessário
}
